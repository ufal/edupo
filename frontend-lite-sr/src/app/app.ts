...
export class App implements Updatable
{
    ...
    generate()
    {
        ...
        .then(async (response:EdupoGenResponse) => {
                ...
                // Hide the close button BEFORE making the modal visible so it never flashes
    ...
    closeResult()
    {
        ...
        // inherit across an explicit `visible` child) and the close button stays painted.
        const buttons = document.getElementById("epl_result_buttons");
     ...
     * with a 10 s countdown. Unless the user cancels it, the UI restarts:
